...
**Approach:** 동물 아이콘 타일 그리드에서 같은 아이콘 두 개를 탭해 매칭·제거하는 퍼즐을 새 페이지로 구현한다. 클리어 시 로컬(localStorage) 재화를 지급하고 다음 스테이지로 진행하며, 스테이지가 오를수록 타일 수를 완만히 늘린다. `[2026-08-11 재협상]` v1은 "항상 화면에 노출된(숨김·암기 없는)" 그리드였으나, 제품 컨셉이 **"기억력 + 반응속도"** 로 재정의되면서 스테이지 4부터 타일이 순차적으로 숨겨지는 하이브리드 모델로 확장됐다 — 아래 Always 항목 참고.

## Boundaries & Constraints

**Always:**
- 스테이지 1~3(`GRACE_STAGE_LIMIT`)에서는 모든 타일이 항상 화면에 보여야 한다 — 온보딩 구간이다.
- 스테이지 4부터는 **순차 숨김(타임어택)** 이 적용된다: 스테이지 시작 시엔 전부 보이고, 시간이 지날수록 타일이 하나씩 숨겨진다. 빨리 맞출수록 정보가 많이 남는 구조라 반응속도가 직접 보상된다. `[2026-08-11 재협상, 마스터 승인]` v1의 "뒤집기/암기 메커닉 금지"를 대체 — PRD §5의 "기억력 게임 유형은 non-goal" 조항도 함께 폐기되고 컨셉이 재서술됐다.
- 순차 숨김에는 **상한 비율**이 있어야 한다 — 남은 타일의 최대 N%(초안 70%)까지만 숨기고 나머지는 항상 노출을 유지한다. 마지막 페어가 둘 다 숨겨져 실력과 무관한 순수 추측만 남는 구간을 만들지 않기 위함이다.
- 숨겨진 타일을 탭하면 **선택은 되되 내용은 공개되지 않는다** — 두 장을 모두 고른 뒤에야 결과가 공개된다.
- 숨김 상태는 **형태로 표시한다**(물음표·빈 테두리 등) — 색상만으로 구분하지 않는다. `DESIGN.md`/`EXPERIENCE.md`의 "매칭 판별은 형태로만, 색상 단독 신호 금지" 원칙을 그대로 따른다.
...
**Acceptance Criteria:**
- Given 유저가 처음 퍼즐 페이지에 진입, when 스테이지 1 보드가 로드되면, then 모든 타일이 숨김 없이 항상 보이는 상태로 렌더된다. `[2026-08-11 재협상]` 이 기준은 유예구간(스테이지 1~3)에 한정된다 — 스테이지 4 이상의 순차 숨김 동작에 대한 acceptance는 별도 구현 스토리에서 정의한다.
...
- **2026-08-11, 컨셉 전환 — 순차 숨김 도입 (human-approved, frozen 섹션 renegotiation)** — 2026-08-11 iOS 실기기 플레이테스트에서 그리드·타이머 두 축이 스테이지 ~10~13 이후 완전히 평평해져 후반이 단조롭다는 피드백이 나왔고, 제품 컨셉을 **"기억력 + 반응속도"** 로 좁히기로 결정. 이는 frozen Boundaries의 **"모든 타일은 항상 화면에 보여야 한다 — 뒤집기/암기 메커닉 금지"** 를 정면으로 뒤집는 사안이라 마스터에게 재협상 승인받음. 확정 내용:
  - **메커닉**: 순차 숨김(타임어택) — 시작 시 전부 노출, 시간 경과에 따라 타일이 하나씩 숨겨짐. 빨리 맞출수록 정보가 남아 반응속도가 직접 보상되고, 늦어질수록 기억에 의존. 두 축이 하나의 메커닉에서 결합.
  - **전환 시점**: 스테이지 4부터(기존 `GRACE_STAGE_LIMIT=3` 재사용, 새 상수 만들지 않음).
  - **숨김 상한 비율**: 남은 타일의 최대 N%(초안 70%)까지만 — 마지막 페어가 둘 다 숨겨지는 "순수 추측" 구간 방지.
  - **숨겨진 타일 탭**: 선택은 되되 내용은 두 장 다 고른 뒤 공개.
  - **접근성**: 숨김 상태를 형태(물음표·빈 테두리)로 표시, 색상 단독 신호 금지 원칙 유지 → **UX 문서 수정 불필요**.
  - **PRD §5 "기억력 게임 유형은 non-goal" 조항은 폐기**하고 컨셉을 재서술한다 — 문구 수정이 아니라 제품 정체성 변경이라 PRD 갱신이 동반됨.
  - **`spec-pattern-match-difficulty.md`의 `MAX_TILES` 24→36 확장은 폐기**(그리드 고정 방침과 충돌). 단 같은 스펙의 **아이콘 그룹핑 + 보조 도형 배지 축은 유효하게 유지**되며 순차 숨김과 같은 접근성 원칙 위에서 결합한다.
  - **실제 코드 구현은 아직 하지 않음** — 별도 구현 스토리로 분리(`deferred-work.md` "[플레이테스트 2026-08-11] ① 난이도 재설계" 항목). 속도 커브·상한 비율 실수치는 플레이테스트로 튜닝. 착수 시 이 스펙을 갱신하거나 새 스펙 작성.
## Design Notes
