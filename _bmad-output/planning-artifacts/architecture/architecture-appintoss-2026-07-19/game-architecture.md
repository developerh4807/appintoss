...
**[NEW 2026-07-19, 마스터 확정 — `[SUPERSEDED 2026-07-19]`]** 검증 과정에서 `TILE_ICONS`가 12종 고정이라 그리드 상한(12쌍) 도달 이후 "패턴 종류 수 증가"라는 난이도 축이 실제로 존재하지 않음을 발견(스테이지 10 이후 보드가 완전히 고정됨). ~~`TILE_ICONS`를 20~30종으로 확장하기로 결정~~ — 같은 날 오후 게임 디자이너 세션에서 **아이콘 풀은 12종 유지 + 실루엣 그룹핑 + 보조 도형 배지로 대체**됨(아이콘 종류만 늘리는 건 새 스킬을 요구하지 않아 저효율이라는 판단). 배지 축은 최초 색상 변주로 검토됐으나 같은 날 진행한 implementation-readiness 체크에서 UX 접근성 원칙("매칭 판별은 형태로만, 색상 단독 신호 금지")과 충돌함이 발견돼 도형 배지로 확정. `MAX_TILES`도 24 → 36으로 상향. 최신 근거는 PRD `addendum.md`("퍼즐 난이도(그리드 크기 + 아이콘 유사도 + 보조 도형 배지) 밸런스 초안" 섹션, 2026-07-19 재설계분)와 `decision-log.md` 2026-07-19 항목 참고.
